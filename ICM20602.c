...
struct icm20602_scale_entry {
	int uscale;     /* scale 鐨勫井灏忔暟閮ㄥ垎锛岄厤鍚? IIO_VAL_INT_PLUS_MICRO */
	u8 regval;      /* 瑕佸啓鍏ュ瘎瀛樺櫒閲忕▼浣嶇殑鍊? */
};
...
static const struct icm20602_scale_entry icm20602_gyro_scale_table[] = {
	{ 133,  0x00 }, /* 卤250 dps锛屽厛鐢? micro 杩戜技 */
	{ 266,  0x08 }, /* 卤500 dps */
	{ 532,  0x10 }, /* 卤1000 dps */
	{ 1064, 0x18 }, /* 卤2000 dps */
};
...
struct icm20602_data{
    ...
    /* 榛樿瀵勫瓨鍣ㄩ厤缃ā鏉匡紝鍙 DT 瑕嗙洊鍚庡啀搴旂敤 */
    struct icm20602_config config;

    /* 褰撳墠杩愯鏃剁姸鎬? */
	int accel_scale_idx;     /* 褰撳墠 accel 閲忕▼鍦ㄨ〃涓殑绱㈠紩 */
	int gyro_scale_idx;      /* 褰撳墠 gyro 閲忕▼鍦ㄨ〃涓殑绱㈠紩 */
	int sampling_frequency;  /* 褰撳墠 ODR / 閲囨牱棰戠巼锛屽崟浣? Hz */

    /* buffer / trigger / fifo 鐩稿叧杩愯鏃剁姸鎬? */
	bool buffer_enabled;
...
static const struct icm20602_config icm20602_default_config = {
    .pwr_mgmt_1         = 0x01,           //鏃堕挓璁剧疆
    .pwr_mgmt_2         = 0x00,          //寮?鍚檧铻轰华鍜屽姞閫熷害璁?
    .config             = 0x01,           //176HZ 1KHZ
    .smplrt_div         = 0x07,           //閲囨牱閫熺巼 SAMPLE_RATE = INTERNAL_SAMPLE_RATE / (1 + SMPLRT_DIV)
    .gyro_config        = 0x18,           //卤2000 dps
    .accel_config       = 0x10,           //卤8g
...
static int icm20602_soft_reset(struct icm20602_data *data)
{
    ...
    /*
     * 瀹樻柟浠ｇ爜鏄? Delay_ms(2) 鍚庡惊鐜 PWR_MGMT_1锛?
     * 鐩村埌璇诲埌 0x41銆?
     */
...
{   
    ...
    //瀹樻柟浠ｇ爜鍦ㄥ瘎瀛樺櫒鍒濆鍖栧墠浼戠湢2ms 鎴戜滑鎹㈡垚杞欢澶嶄綅
